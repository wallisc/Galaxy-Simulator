...
struct VSGalaxyDrawOut
{
    float3 pos			: POSITION;
    float4 color		: COLOR;
};

struct GSGalaxyDrawOut
{
...
cbuffer cb1
{
    static float g_fParticleRad = 4000.0f;   
};
...
//
VSGalaxyDrawOut VSGalaxyDraw(VSParticleIn input)
{
    VSGalaxyDrawOut output;
...
//
[maxvertexcount(4)]
void GSGalaxyDraw(point VSGalaxyDrawOut input[1], inout TriangleStream<GSGalaxyDrawOut> SpriteStream)
{
    GSGalaxyDrawOut output;
...
//
float4 PSGalaxyDraw(PSGalaxyDrawIn input) : SV_Target
{   
